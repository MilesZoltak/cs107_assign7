...
static size_t eval_correctness(script_t *script, bool quiet, bool *success) {
    *success = false;
    ...
    for (int req = 0; req < script->num_ops; req++) {
        ...
        if (script->ops[req].op == ALLOC) {//if we are allocing
            bool fail = false;
            void *p = eval_malloc(req, requested_size, script, &fail);
            //eval_malloc will tell us if we failed
            if (fail) {
                return -1;
            }
            ...
            //this advances *heap_end by the number of bytes we are requesting
            if ((char *)p + requested_size > (char *)heap_end) {
                heap_end = (char *)p + requested_size;
            }
        } else if (script->ops[req].op == REALLOC) {//if we are reallocing
            ...
            cur_size += (requested_size - old_size);//remember, even though they request n, we aren't adding n bytes... think!
            if ((char *)p + requested_size > (char *)heap_end) {
                heap_end = (char *)p + requested_size;
            }
        } else if (script->ops[req].op == FREE) {//if not the above, then we are freeing (but don't assume so (elseif))
            size_t old_size = script->blocks[id].size;
            void *p = script->blocks[id].ptr;

            // verify payload intact before free (FIGURE OUT WTF THIS MEANS)
            if (!verify_payload(p, old_size, id, script, 
                script->ops[req].lineno, "freeing")) {
                return -1;
            }
            script->blocks[id] = (block_t){.ptr = NULL, .size = 0};//update our array (i think this is symbollic of myfree)
            myfree(p);//WAIT WHY ARE WE CALLING MYFREE HERE BUT NOT MYMALLOC/MYREALLOC ABOVE????
            cur_size -= old_size;
        }

        // check heap consistency after each request and stop if any error
        /* at this point, we check for validity again to see if a call we made messed anything up... */
        if (!quiet && !validate_heap()) {
        ...
        //peak size gets updated as we go i guess, we only add into it if we ever surpass, never decrement (is basically a ceiling)
    ...
    /* what the heck does payload intact mean and how is that different from a valid heap???*/
...
static void *eval_malloc(int req, size_t requested_size, script_t *script, 
    ...
    //make sure we have size for block (if mymalloc returns Null and we are asking for >0 bytes.. we got problem)
    void *p;
...
static void *eval_realloc(int req, size_t requested_size, script_t *script, 
    ...
    //verify the old payload is intact before we work w it
    ...
    //verify the new neighborhood is safe for the data we are moving
    ...
    /*this works i guess because above we already did the realloc in checking for space... if it worked, it's there*/
...
static bool verify_block(void *ptr, size_t size, script_t *script, int lineno) {
    ...
    for (int i = 0; i < script->num_ids; i++) {
        ...
        //i'm pretty confused by this logic...
...
static bool verify_payload(void *ptr, size_t size, int id, script_t *script, 
    ...
    //on creation, block was filled with predictable info, if that info has been changed, signal an issue
